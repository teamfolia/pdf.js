...
class FoliaAPI {
  ...
  async #api_request(path, body, method = "POST") {
    ...
    if (this.user && this.user["auth"]) {
      url += /\?/g.test(path) ? `&v=8&auth=${this.user["auth"]}` : `?v=8&auth=${this.user["auth"]}`;
    }
  ...
  getPageAnnotations(workspaceId, documentId, pageNumber) {
  ...
  async cc(workspaceId, object = 0, mod = 0) {
    ...
    let objects =
      workspaceData.objects instanceof Array ? workspaceData.objects : Object.values(workspaceData.objects);
    ...
    let workspace = {
      cid: workspaceId,
      mod: workspaceData.mod,
      object: workspaceData.object,
      perms: workspaceData.perms,
    };
      ...
      switch (objData.type) {
        ...
        case "comment": {
          ...
          for (const card of workspace.cards) {
            for (const anno of card.annotations) {
              if (anno.object_id === objData.context) {
                annotation = anno;
                // console.log('-->', objData, annotation)
                break;
              }
            }
            if (annotation) break;
          ...
          annotation.comments.push({ ...objData, localId: `${annotation.comments.length}` });
          break;
        }
        default:
          break;
    ...
    if (
      this.workspace &&
      (this.workspace.mod !== workspace.mod || this.workspace.object !== workspace.object)
    ) {
      hasChanges = true;
    }
  ...
  getUserInfo = (email) => {
    const user = this.workspace.perms.find((user) => user.email === email);
    return { ...user, avatarUrl: user ? `${PUBLISH_SERVER}/user/avatar/${user.avatar}/?v=8` : null };
  };
        ...
        .then(({ workspace, hasChanges }) => {
          ...
          if (hasChanges && this.eventBus) {
            this.eventBus.dispatch("folia:has_changes", {
              workspaceId: this.workspace.cid,
              mod: workspace.mod,
